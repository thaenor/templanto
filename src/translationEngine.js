...
export function validateXLSx(file) {
  try {
...
export function getXLSxData(file) {
  try {
    let final = [];
    const workbook = XLSX.readFile(file);
    for (let i = 0; i < workbook.SheetNames.length; i++) {
...
export function createFolderIfExists(location, name) {
...
export function main() {
  ...
  for (let i = 0; i < translation_xls.length; i++) {
    if (translation_xls[i] === '.gitignore' || translation_xls[i] === '.DS_Store') {
      continue;
    } //ignoring .gitignore heheh
    try {
...
export function generateTranslations(input_file) {
...
export function render() {
  ...
  for (let i = 0; i < templates.length; i++) {
    if (templates[i] === '.gitignore' || templates[i] === '.DS_Store') {
      continue;
    } //ignoring .gitignore heheh
